...
<style>
    ...
    .pesanan-card:hover {
        transform: translateY(-5px); /* Gerakan ke atas saat hover */
    }
    ...
    .total-harga {
        font-weight: bold;
        color: #d9534f; /* Warna merah untuk menonjolkan harga */
        font-size: 1.2em;
    }
</style>
        ...
        <div class="pesanan-card">
            <ul>
                <li>
                    ...
                    <span class="label">Jadwal:</span> {{ $order->jadwal->Waktu_Pemberangkatan }} - {{ $order->jadwal->Waktu_Tiba }}<br>
                    <span class="label">Kursi:</span> 
                    @foreach($order->kursis as $kursi)
                        {{ $kursi->Nomor_Kursi }}@if(!$loop->last), @endif
                    @endforeach
                    <br>
                    <span class="label total-harga">Total Harga: Rp{{ number_format($order->Total_Harga, 0, ',', '.') }}</span>
                </li>
            </ul>
        </div>
    @endforeach
